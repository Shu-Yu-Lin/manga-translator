...
def load_dictionary(file_path):
    """
    Load one dictionary file, or several layered on top of each other.

    Several paths are applied in order, so a shared dictionary can be passed
    first and a per-book one after it to add names only that book needs.
    """
    if isinstance(file_path, (list, tuple)):
        dictionary = []
        for path in file_path:
            dictionary.extend(load_dictionary(path))
        return dictionary
...
def apply_post_dictionary(regions, post_dict):
    """Apply `post_dict` to every region's translation, logging what changed."""
    replacements = []
    for region in regions:
        ...
        if original != region.translation:
            replacements.append(f"{original} => {region.translation}")
...
class MangaTranslator:
    ...
    def __init__(self, params: dict = None):
        ...
        self._batch_contexts = []  # contexts for the batch
        self._batch_configs = []   # configs for the batch
        self.disable_memory_optimization = params.get('disable_memory_optimization', False)
        # batch_concurrent  parse_init_params
        ...
        self._original_page_texts = []  # source pages, used as context in concurrent mode

        # Debug image bookkeeping
        self._current_image_context = None  # context of the image being processed
        self._saved_image_contexts = {}     # per-image context within a batch
        
        self._setup_log_file()

    def _setup_log_file(self):
        """Create a timestamped log file under the result folder."""
        try:
            # result
            ...
            # Match the console formatting
            from .utils.log import Formatter
            formatter = Formatter()
            file_handler.setFormatter(formatter)
            
            # manga-translatorlogger
            ...
            try:
                ...
                _global_console = display_console  # console output
                _log_console = log_console         # log file
    ...
    def parse_init_params(self, params: dict):
        ...
        if self.batch_concurrent and self.batch_size < 2:
            ...
            # Concurrency disabled automatically
    ...
    def _set_image_context(self, config: Config, image=None):
        """Record which image is being processed, for naming debug subfolders."""
        from .utils.generic import get_image_md5

        # Millisecond timestamp keeps the name unique
        ...
        # {timestamp}-{file_md5}-{detection_size}-{target_lang}-{translator}
    ...
    def _get_image_subfolder(self) -> str:
        """Debug subfolder name for the image being processed."""
    ...
    async def translate(self, image: Image.Image, config: Config, image_name: str = None, skip_context_save: bool = False) -> Context:
        ...
        # Name the debug subfolder after this image
        ...
        # Save at the very end, so retries are included
        ...
        if not skip_context_save and ctx.text_regions:
            # Summarise this page as context for the next
    ...
    async def _translate(self, config: Config, ctx: Context) -> Context:
        ...
        if self.verbose and ctx.text_regions:
            show_panels = not config.force_simple_sort  # panel
        ...
        if hasattr(self, '_progress_hooks') and self._current_image_context:
            ...
            # Formatted so a front-end can parse it
    ...
    async def _revert_upscale(self, config: Config, ctx: Context):
        ...
        if ctx.result and not self.result_sub_folder and hasattr(self, '_is_streaming_mode') and self._is_streaming_mode:
            ...
            # Tell the front-end the file is ready
            if hasattr(self, '_progress_hooks') and self._current_image_context:
                folder_name = self._current_image_context['subfolder']
    ...
    async def _run_textline_merge(self, config: Config, ctx: Context):
        ...
        for region in text_regions:
            ...
            if has_brackets:  
                ...
                # First pass: mark matched brackets  
                ...
                for i, char in enumerate(stripped_text):  
                    if char in left_symbols:  
                        ...
                        if stack:  
                            ...
                            # No matching opener: mark for removal  
                            # No corresponding left parenthesis, marked for deletion
                            to_skip.append(i)  
                
                # Mark unmatched openers for removal
                ...
                # Second pass: fix brackets that matched the wrong form
                # Second pass: Process matching but mismatched brackets
                stack = []  
                for i, char in enumerate(stripped_text):  
                    if i in to_skip:  
                        # Skip an orphaned bracket
                    ...
                    if char in left_symbols:  
                        ...
                        if stack:  
                            ...
                            if char != expected_right:  
                                # Give an unmatched closing bracket the form its opener expects
    ...
    async def _run_text_translation(self, config: Config, ctx: Context):
        ...
        # prep_manualtranslatornonetoken
        ...
        if config.translator.translator == Translator.none:  
            ...
            for region in ctx.text_regions:  
                region.translation = ""  # an empty translation blanks the region  
        ...
        if config.translator.translator != Translator.none or not self.prep_manual:  
            for region, translation in zip(ctx.text_regions, translated_sentences):  
                if config.render.uppercase:  
                    ...
                    translation = translation.lower()  # lowerupper
        ...
        # Results are saved at the end of the flow, after any retries
        ...
        if config.translator.enable_post_translation_check:
            ...
            for region in ctx.text_regions:
                if region.translation and region.translation.strip():
                    # Hallucination check only, no page-level language check
        ...
        # Post-translation stage two: page-level language check on the filtered regions
        if config.translator.enable_post_translation_check:
            
            # Page-level target language check
            ...
            if ctx.text_regions and len(ctx.text_regions) > 5:
                ...
                if not page_lang_check_result:
                    ...
                    # Stage two: re-translate the whole batch
        ...
        # Apply the post dictionary last: the check above re-translates regions it
        # rejects, which used to throw away the replacements when they ran first.
        apply_post_dictionary(ctx.text_regions, load_dictionary(self.post_dict))
    ...
    async def translate_batch(self, images_with_configs: List[tuple], batch_size: int = None, image_names: List[str] = None) -> List[Context]:
        """
        Translate several images, batching the translation stage.
        Args:
            images_with_configs: List of (image, config) tuples
            batch_size: batch size; None uses the instance's batch_size
            image_names: deprecated, kept for compatibility
        Returns:
            List of Context objects with translation results
        """
        batch_size = batch_size or self.batch_size
        if batch_size <= 1:
            # Batching disabled: one image at a time
        ...
        for i, (image, config) in enumerate(images_with_configs):
            ...
            try:
                self._set_image_context(config, image)
                # Keep the context so post-processing writes to the same folder
                if self._current_image_context:
        ...
        try:
            ...
            # Fall back to translating one page at a time
            translated_contexts = []
            for ctx, config in pre_translation_contexts:
                try:
                    if ctx.text_regions:  # text_regionsNone
        ...
        for i, (ctx, config) in enumerate(translated_contexts):
            try:
                if ctx.text_regions:
                    # Reuse the context from the pre-processing stage
                    # Recover the context by hashing the image
                    from .utils.generic import get_image_md5
                    image = ctx.input  # context
    ...
    async def _translate_until_translation(self, image: Image.Image, config: Config) -> Context:
        """
        Everything before translation: upscaling, detection, OCR, textline merge.
        """
        ctx = Context()
        ctx.input = image
        ctx.result = None
    ...
    async def _batch_translate_contexts(self, contexts_with_configs: List[tuple], batch_size: int) -> List[tuple]:
        """
        Translate in batches to keep memory bounded.
        """
        results = []
        ...
        for i in range(0, total_contexts, batch_size):
            ...
            all_texts = []
            batch_text_mapping = []  # contextregion
            ...
            if not all_texts:
                # Nothing to translate in this batch
                results.extend(batch)
                continue
                
            try:
                await self._report_progress('translating')
                # Configs are identical within a batch, so use the first
                ...
                if sample_config:
                    ...
                    translated_texts = all_texts  # keep the original when translation is impossible
                    
                # context
                text_idx = 0
                for ctx_idx, (ctx, config) in enumerate(batch):
                    if not ctx.text_regions:  # text_regionsNone
                        continue
                    for region_idx, region in enumerate(ctx.text_regions):
                    ...
                    if all_batch_regions and len(all_batch_regions) > 10:
                        ...
                        if not batch_lang_check_result:
                            ...
                            while batch_retry_count < max_batch_retry and not batch_lang_check_result:
                                ...
                                # Re-translate every region in the batch
                                all_original_texts = []
                                region_mapping = []  # textctx
                ...
                # Apply the post dictionary last, after any re-translation above.
                post_dict = load_dictionary(self.post_dict)
                for ctx, config in batch:
                    apply_post_dictionary(ctx.text_regions, post_dict)
    ...
    async def _concurrent_translate_contexts(self, contexts_with_configs: List[tuple]) -> List[tuple]:
        """
        Translate concurrently, one request per image, instead of one huge batch.
        """

        batch_original_texts = []  # source texts for this batch
        ...
        async def translate_single_context(ctx_config_pair_with_index):
            ...
            try:
                ...
                # Translate this image on its own
                ...
                if config.translator.enable_post_translation_check and ctx.text_regions:
                    page_lang_check_result = await self._check_target_language_ratio(
                        ctx.text_regions,
                        config.translator.target_lang,
                        min_ratio=0.3  # looser threshold for a single page
                    )
        ...
        for i, ctx_config_pair in enumerate(contexts_with_configs):
            # This page's index across the whole run
            page_index = len(self.all_page_translations) + i
            batch_index = i  # index within the batch
    ...
    async def _batch_translate_texts(self, texts: List[str], config: Config, ctx: Context, batch_contexts: List[Context] = None, page_index: int = None, batch_index: int = None, batch_original_texts: List[dict] = None) -> List[str]:
        """
        Translate a list of texts through the normal translator interface.

        Args:
            texts: the texts to translate
            config: the config object
            ctx: the context object
            batch_contexts: the contexts in this batch
            page_index: index of this page, used for context in concurrent mode
            batch_index: index of this page within the batch
            batch_original_texts: source texts for this batch
        """
        if config.translator.translator == Translator.none:
    ...
    async def _apply_post_translation_processing(self, ctx: Context, config: Config) -> List:
        """
        Post-process translations: bracket repair, filtering.
        """
        # text_regionsNone
        if not ctx.text_regions:
            return []
    ...
    async def _complete_translation_pipeline(self, ctx: Context, config: Config) -> Context:
        """
        Post-translation stages: mask refinement, inpainting, rendering.
        """
        await self._report_progress('after-translating')
    ...
    async def _check_repetition_hallucination(self, text: str, threshold: int = 5, silent: bool = False) -> bool:
        """
        Detect repeated content, which signals a hallucinating model.
        ...
        # Word-level repetition (CJK splits per character, others on spaces)
        ...
        if len(words) >= threshold * 2:
            for i in range(len(words) - threshold + 1):
                ...
                if phrase in remaining_text:
                    ...
                    if phrase_count >= 3:  # lowered phrase-repetition threshold
                        if not silent:
    ...
    async def _check_target_language_ratio(self, text_regions: List, target_lang: str, min_ratio: float = 0.5) -> bool:
        """
        Check how much of the translation is actually in the target language.
        ...
        Args:
            text_regions: the text regions
            target_lang: target language code
            min_ratio: unused, kept for compatibility
            
        Returns:
            bool: True if the check passed
        """
        if not text_regions or len(text_regions) <= 10:
            # Too few regions for the ratio to mean anything
            return True
        ...
        # Detect the language over the whole page at once
    ...
    async def _validate_translation(self, original_text: str, translation: str, target_lang: str, config, ctx: Context = None, silent: bool = False, page_lang_check_result: bool = None) -> bool:
        """
        Validate a translation: target-language ratio plus hallucination detection.
        Validate translation quality (includes target language ratio check and hallucination detection)
        
        Args:
            page_lang_check_result: result of the page-level check; None means run it
        """
        ...
        # 1. Page-level target language ratio
    ...
    async def _retry_translation_with_validation(self, region, config: Config, ctx: Context) -> str:
        """
        Re-translate a region until it passes validation.
        ...
        for attempt in range(max_attempts):
            ...
            is_valid = await self._validate_translation(
                ...
                config,
                ctx=None,  # ctx
                silent=True,  # quiet while retrying
                page_lang_check_result=True  # Trueregion
            )
