...
def play_completion_sound():
    ...
    try:
        ...
        if platform.system() == 'Windows':
            ...
            # Other platforms: console bell
            print('\a', end='', flush=True)
    except Exception as e:
        # A failed beep must never stop a translation run
        logger.debug(f'Failed to play completion sound: {e}')

def safe_get_memory_info():
    """Memory usage, falling back to pessimistic values if it cannot be read."""
    try:
        memory = psutil.virtual_memory()
        return memory.percent, memory.available // (1024 * 1024)  # available MB
    except Exception as e:
        logger.warning(f'Unable to get memory info: {e}')
        return 95.0, 100  # Assume memory is tight, so callers free aggressively

def force_cleanup():
    """Release memory held by Python and the GPU allocator."""
...
class MangaTranslatorLocal(MangaTranslator):
    ...
    async def _translate_folder_batch(self, path: str, dest: str, params: dict, config: Config, file_ext: str):
        """Translate a folder of images in batches."""
        ...
        # Collect the images to translate
        image_tasks = []
        for root, subdirs, files in os.walk(path):
            files = natural_sort(files)
            ...
            for f in files:
                ...
                try:
                    ...
                    img.verify()
                    img = Image.open(file_path)  # verify() closes the file, so reopen it
        ...
        while i < len(image_tasks):
            ...
            # Free memory before starting a batch if it is running low
            ...
            try:
                ...
                for j, (ctx, (img, _, file_path, output_dest)) in enumerate(zip(batch_results, batch)):
                    # Honour --skip-no-text for images where nothing was found
                    ...
                    if ctx and ctx.result:
                        ...
                        # No result: tell apart "nothing detected" from "translation failed"
                        ...
                        if not ctx:
                            ...
                            if has_original_text:
                                # Text was found but every region was filtered out
                                filtered_by_processing = all(
                                    hasattr(region, 'translation') and 
                                    (not region.translation.strip() or  # empty
                                     region.translation.isnumeric() or  # numeric only
                                     region.text.lower().strip() == region.translation.lower().strip())  # unchanged from the original
                        ...
                        if save_reason == "translation_failed_with_text":
                            # Translation genuinely failed - leave no output so a rerun retries it
                            should_save = False
                            # logger.info(f'Skipping save for retry: {file_path} (translation failed but has original text)')
                        
                        # Nothing to translate: copy the original through
                        if should_save and not self.skip_no_text:
                            ...
                            try:
                                ...
                                if self.save_quality and self.save_quality < 100:
                                    # A quality setting means re-encode as JPEG
                                ...
                                translated_count += 1  # an untranslated copy still counts as processed
