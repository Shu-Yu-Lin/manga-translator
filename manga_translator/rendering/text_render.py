...
def calc_vertical(font_size: int, text: str, max_height: int):
    ...
    for i, cdpt in enumerate(text):
        ...
        # Never start a new column with punctuation that belongs to the line above
        # (kinsoku shori) - let that one character overflow instead.
        if line_height + char_offset_y > max_height and cdpt not in CJK_BREAK_AFTER:
...
def put_char_vertical(font_size: int, cdpt: str, pen_l: Tuple[int, int], canvas_text: np.ndarray, canvas_border: np.ndarray, border_size: int):  
    """  
    Draw one character vertically on the canvas, optionally with an outline.  
    Vertically place a character on the canvas with optional border effect.  
    
    Args:  
        font_size: / Font size
        cdpt: / Character to render
        pen_l: / Pen position (starting drawing position)
        canvas_text: NumPy / NumPy array for drawing text
        canvas_border: NumPy / NumPy array for drawing border
        border_size: / Border size
        
    Returns:  
        int: / Vertical advance value
    """  
    # Copy the pen so the caller's position is not modified  
    ...
    bitmap = slot.bitmap  #  bitmap  / This is the bitmap object of the original character

    # --- / Get original character bitmap information ---
    char_bitmap_rows = bitmap.rows  
    char_bitmap_width = bitmap.width  
    
    # Spaces and similar characters have no bitmap  
    ...
    if char_bitmap_rows * char_bitmap_width == 0 or len(bitmap.buffer) != char_bitmap_rows * char_bitmap_width:  
        ...
        # Blank characters only contribute their advance  
        # For whitespace characters, just return the vertical advance  
        return char_offset_y  

    # --- / For valid bitmaps, process normally ---
    # char_offset_y
    ...
    # paste_y_start paste_y_end paste_y_start:paste_y_end 0
    # Crude guard - can drop the occasional character
    ...
    # Source and destination slices must be the same size  
    ...
    if border_size > 0:  
        ...
        stroke_radius = 64 * max(int(0.07 * font_size), 1)  #  / Proportional value based on font size
        ...
        bitmap_b = blyph.bitmap  #  bitmap  / This is the bitmap object after stroking

        # --- / Get border bitmap information ---
        ...
        if border_bitmap_rows * border_bitmap_width > 0 and len(bitmap_b.buffer) == border_bitmap_rows * border_bitmap_width:  
            ...
            # Outline centre, relative to its top-left  
            ...
            # Glyph centre in canvas coordinates  
            ...
            # --- canvas_border / Place border bitmap on canvas_border ---
            # Keep the paste region inside the canvas  
            ...
            if paste_border_y_start >= paste_border_y_end or paste_border_x_start >= paste_border_x_end:  
                ...
                if bitmap_border_slice.size > 0:
                    ...
                    if target_slice.shape == bitmap_border_slice.shape:  
                        ...
                        # Shapes disagree - skip rather than crash  
...
# Characters a CJK line may end on. Breaking right after one of these reads as a
# natural pause; breaking anywhere else splits a word (/) or a term (/).
CJK_BREAK_AFTER = '。．、，！？；：…～)）」』】〉》”’'


def split_into_words(text: str) -> List[str]:
    """
    Split text into the units the line-wrapper is allowed to break between.

    ponytail: CJK writes without spaces, so splitting on whitespace alone hands
    the wrapper one huge "word" and it falls back to breaking between arbitrary
    characters. Splitting after CJK punctuation as well gives it real break
    points; anything still too long for a line is split per character downstream.
    """
    words = []
    for chunk in re.split(r'\s+', text):
        if not chunk:
            continue
        words.extend(w for w in re.split(f'(?<=[{re.escape(CJK_BREAK_AFTER)}])', chunk) if w)
    return words
...
def calc_horizontal(font_size: int, text: str, max_width: int, max_height: int, language: str = 'en_US', hyphenate: bool = True) -> Tuple[List[str], List[int]]:
    ...
    for i, line in enumerate(line_words_list):
        ...
        for j, word_idx in enumerate(line):
            ...
            # CJK segments were split at punctuation, not at spaces - joining them
            # with a space would open a gap mid-sentence.
            if j < len(line) - 1 and len(line_text) > 0 and line_text[-1] not in CJK_BREAK_AFTER:
                line_text += ' '
...
def put_char_horizontal(font_size: int, cdpt: str, pen_l: Tuple[int, int], canvas_text: np.ndarray, canvas_border: np.ndarray, border_size: int):
    ...
    Draw one character horizontally on the canvas, optionally with an outline.

    Args:
        font_size: Font size in pixels.
        cdpt: Character to render.
        pen_l: Current pen position (x, y), where x is horizontal origin and y is baseline. 
               Current pen position (x, y): x is the horizontal origin, y the baseline.
        ...
        border_size: Target stroke size (used to calculate stroker radius, enabled when >0).
                    Outline size; any value above 0 enables the outline pass.

    Returns:
        The character's horizontal advance distance (int). (int)
    """
    pen = list(pen_l)  # Use mutable copy
    ...
    bitmap = slot.bitmap  # Original character bitmap
    ...
    # No metrics: estimate from bitmap width and left bearing
    elif bitmap.width > 0 and hasattr(slot, 'bitmap_left'):
         char_offset_x = slot.bitmap_left + bitmap.width  # Rough estimation
    
    # Final fallback: Guess based on font size
    # Last resort: guess from the font size
    else:
         char_offset_x = font_size // 2  # If no information available

    # --- Check bitmap validity ---
    # buffer
    # Handle spaces/invalid chars before accessing buffer
    if bitmap.rows * bitmap.width == 0 or len(bitmap.buffer) != bitmap.rows * bitmap.width:
        return char_offset_x  # Return advance for empty/invalid bitmap
    ...
    # ( border_size > 0)
    if border_size > 0:
        # Get glyph outline for stroke
        glyph_border = get_char_border(cdpt, font_size, 0)  # Same horizontal orientation
        
        # Configure stroker
        stroker = freetype.Stroker()
        stroke_radius = 64 * max(int(0.07 * font_size), 1)  # In 1/64 pixel units : 1/64
        stroker.set(stroke_radius, 
                   freetype.FT_STROKER_LINEJOIN_ROUND,  # Round joins
                   freetype.FT_STROKER_LINECAP_ROUND,   # Round line caps
                   0)
        ...
        bitmap_b = blyph.bitmap  # Stroked bitmap
        ...
        # Only continue when the outline bitmap is usable
        if (border_bitmap_rows * border_bitmap_width > 0 and 
            ...
            if (bitmap_border_slice.size > 0 and 
                ...
                # Shapes should already match if the slicing above is right
                if target_slice.shape == bitmap_border_slice.shape:
    ...
    return char_offset_x  # Return horizontal advance
